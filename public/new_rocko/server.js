...
var sessionChecker = (req, res, next) => {
    if (req.session.user && req.cookies.user_sid) {
        res.redirect('/2_pag_principal');
    } else {
        next();
    }
};
app.get('/', sessionChecker, (req, res) => {
    res.redirect('/1_plantilla_Login');
});
app.get('/2_pag_principal', (req, res) => {
    registrarUso(req.session.user, '/2_pag_principal');
    if (req.session.user && req.cookies.user_sid) {
        res.sendFile(__dirname + '/public/2_pag_principal.html');
    } else {
        res.redirect('/1_plantilla_Login');
    }
});
//-----------------esta pag no existe en el new Rocko-----------------
...
//--------------------------------------------------------------------
app.get('/3_registrar_Actividad', (req, res) => {
    registrarUso(req.session.user, '/3_registrar_Actividad');
    if (req.session.user && req.cookies.user_sid) {
        res.sendFile(__dirname + '/public/3_registrar_Actividad.html');
    } else {
        res.redirect('/1_plantilla_Login');
    }
});
app.get('/5_tranf_Actividades', (req, res) => {
    registrarUso(req.session.user, '/5_tranf_Actividades');
    if (req.session.user && req.cookies.user_sid) {
        res.sendFile(__dirname + '/public/5_tranf_Actividades.html');
    } else {
        res.redirect('/1_plantilla_Login');
    }
});
...
//--------------------------------------------------------------------
app.get('/4_crear_Actividades', (req, res) => {
    registrarUso(req.session.user, '/4_crear_Actividades');
    if (req.session.user && req.cookies.user_sid) {
        res.sendFile(__dirname + '/public/4_crear_Actividades.html');
    } else {
        res.redirect('/1_plantilla_Login');
    }
});
app.get('/6_revisar_Datos', (req, res) => {
    registrarUso(req.session.user, '/6_revisar_Datos');
    if (req.session.user && req.cookies.user_sid) {
        res.sendFile(__dirname + '/public/6_revisar_Datos.html');
    } else {
        res.redirect('/1_plantilla_Login');
    }
});
...
// -------------- lo que aparece en la pag mis actividades vieja :: parece que si----------------
...
// -------------- lo que aparece en la pag mis actividades?----------------
...
app.route('/actividad/crear')  
    ...
    // ---------- modificar respaldo :: hayar respaldo
